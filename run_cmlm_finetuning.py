...
# Import our safer vocabulary loader
...
def update_model_architecture(model, embedding):
    """
    Update model architecture to accommodate new vocabulary size.
    
    Args:
        model: BertForSeq2seq model
        embedding: New embedding matrix
    ...
    logger.info(f"Original model: BERT vocab size = {model.config.vocab_size}")
    logger.info(f"Target vocabulary size = {vocab_size}")
    
    # Update model architecture
    model.cls.predictions.decoder = torch.nn.Linear(hidden_size, vocab_size, bias=True)
    model.cls.predictions.bias = torch.nn.Parameter(torch.zeros(vocab_size))
...
def main(opts):
    ...
    try:
        # Try using the safer vocabulary loading method first
        vocab_dump = safe_load_vocab(opts.vocab_file)
